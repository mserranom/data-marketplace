...
def _upsert_config_item(config, current_user_id):
    item = {
        ...
        "config": config}
    dynamodb.insert(table_name='configs', item=item)
...
def _upsert_polling_subscription(config):
    item = {
        ...
        "url": config["url"]}
    dynamodb.insert(table_name='polling_subscriptions', item=item)
...
def _update_config_tags(config, current_user_id, existing_tags):
    ...
    for tag in existing_tags:
        tag_keys_to_delete.append({
            "tag": tag,
            "config_id": current_user_id + "/" + config["id"]})
    dynamodb.insert_and_delete(table_name='config_tags',
                               items_to_add=tags_to_add,
                               keys_to_delete=tag_keys_to_delete,
                               key_names=['tag', 'config_id'])
